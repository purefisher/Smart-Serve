...
% Test Case Derivation: \wss{The expected output has been derived from the functional requirements listed in the SRS document. These are ODR6 and ODR9.}
...
Test Case Derivation: \wss{The expected output has been derived from the functional requirements listed in the SRS document. These are ODR7 and ODR8.}
...
Output: \wss{Operator can view the order number in the queue of orders.}
...
% Output: \wss{The drink order is updated to the new order. The order number in the queue remains the same.}
...
Output: \wss{Smart Serve searches for a cup by doing a full rotation, waiting 8 seconds and repeating this to give the user 3 chances to place their cup.}%should we mention that system shouldn't pour without a cup

Test Case Derivation: \wss{This test proves that Smart Serve can handle the scenario where a drink is ordered and no cup is present.}
...
Output: \wss{Smart Serve begins making the drink ordered as soon as the cup is sensed. The drink is poured until completion.}

Test Case Derivation: \wss{This test case checks what happens when there is initially no cup after a drink is ordered, and then a cup is placed.}
...
Output: \wss{Completed drink made with the correct proportions. The correct user is removed from the queue.}
...
Test Case Derivation: \wss{If the system is ready, this is the only scenario where the user is removed from the queue.}
...
Output: \wss{Smart Serve checks to see if the cup is full and rejects it as Smart Serve will not pour a drink in a full cup. Smart Serve will continue checking every 8 seconds to see if an empty cup is present to pour into.}
...
Test Case Derivation: \wss{From requirement MDR6 and MDR7 in the SRS, the expected behaviour of our system when there is a completed drink in the pouring area and other drinks in queue, is to not pour the next drink until the completed one is removed.}
...
Output: \wss{Smart Serve checks to see if the cup is considered full and rejects it as Smart Serve will not pour a drink in a full cup. Smart Serve will continue checking every 8 seconds to see if an empty cup is present to pour into.}
%Could add a test where the next order is placed using the same user account, make sure they still have their order completed notification from the first drinks, and still have order info for their next one.
